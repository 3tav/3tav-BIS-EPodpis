...
{
    public class mSignAgentLibECE : ImsignAgentLib
    {
        ...
        public void GeneratePDFMerged(int stevilkaPogodbe)
        {
            ...
            var nazivPDF = string.Format(_fileNameMerged, stevilkaPogodbe);
...
//            }
        ...
        public void SignProSignFile(int id, int userId, string fileName) 
        {
            var dal = new mSignDAL();
            try
            {
                GeneratePDFMerged(id);
            }
            catch (Exception ex)
            {
                try
                {
                    dal.Log("GeneratePDFMerged", id.ToString(), userId, id, -1, ex.Message);
                }
                catch (Exception ex2)
                {
            ...
            catch (Exception ex)
            {
                try
                {
                    dal.Log("SignProSignFile", fileName, userId, id, -1, ex.Message);
                }
                catch (Exception ex3)
                {
        ...
        public void SignProImportSigned(int id, int userId, string filePath)
        {
            
              
            //var filesFilter = string.Format("ece-{0}*.pdf", id);
            var filesFilter = string.Format("*{0}*.pdf", id);

            var tempPath = string.Format("{0}\\epodpis_pdf\\podpisano\\", Path.GetTempPath());
            if (Directory.Exists(tempPath) == false)
                Directory.CreateDirectory(tempPath);

            var di = new DirectoryInfo(tempPath);

            foreach (var f in di.GetFiles(filesFilter))
            {
                ...
                try
                {
                    tip = Convert.ToInt32(fileName.Substring(fileName.LastIndexOf("_") + 1, fileName.IndexOf(".") - fileName.LastIndexOf("_") - 1));
                }
                catch (Exception ex)                 
                { 
                    // silent fail
                ...
                try
                {
                    ...
                    // doc
                    docId = dbp.InsertDoc(fileName, string.Empty, "pdf", File.ReadAllBytes(f.FullName), userId, contentType);
                    // link
                    dbp.InsertDocLinkPogodba(id, docId, fileName, opis, string.Empty, tip, vrsteObrazcev, userId);
                }
                catch (Exception ex)
                {
                    throw new Exception(string.Format("{0} [{1}]", ex.Message, 10));
